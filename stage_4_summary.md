...
> **版本**: v4.0 + 多轮公理分解 + 第6章显式积分核 + 类型化改造 + MEF核心完全拆解  
> **最后更新**: 2026-09-10  
> **文件**: stage_4.lean  
...
1. **Section 0**: 流形类型与 L² 函数空间（新增，类型化改造）
...
8. **Section 6**: 显式积分核基础设施（类型化到 ManifoldM）
9. **Section 7**: Mollified Explicit Formula（A3）
10. **Section 8**: RH 主定理

---

## 四、公理清单（63 条，按 ZFC 可证性分档）
...
11. `heatKernel_commutes_laplacian` — 热核与 Laplacian 交换（True 简化）

**积分/测度（4）**
12. `manifoldIntegral_linear` — 积分线性性
13. `laplaceTransform_exp` — Laplace 变换 e^{-st}
14. `laplaceTransform_linear` — Laplace 变换线性
15. `trace_cyclicity` — 迹循环性（True 简化）

**群作用/几何（4）**
16. `manifoldM_nonempty` — 流形非空（新增）
...
51. `geometricKernelTrace_integral_representation` — 几何核迹积分表示（精确）

**热核函数演算（2）**
52. `fLaplacianKernel_via_heatKernel` — f(Δ) 核热核表示（True 简化）
53. `fLaplacianKernel_heatKernel_exact` — f(Δ) 核热核精确式（True 简化）

**分布论（3）**
54. `distribution_equality_support` — 分布等式 → 支撑相等
55. `spectral_side_support` — 谱侧支撑
56. `zero_side_support` — 零点侧支撑
...
| # | 公理 | 风险 | 说明 |
|---|------|------|------|
| 57 | `mollified_continuous_spectrum_vanishes` | 低 | 已澄清：磨光函数支集分离 → 连续谱为零，非泛泛断言 |
| 58 | `mollified_spectral_delta` | 低 | 插值理论标准结果，与论文定义性对应等价 |
| 59 | `delta_trace_zero_correspondence` | **高** | 需单点 Mellin 局部化，比成对局部化更强 |
| 60 | `zero_side_melin_localization` | 中 | Weil 显式公式零点侧结构 |
| 61 | `melin_pair_sum_nondegenerate` | 中 | Mellin 变换非退化，泛函分析 |
| 62 | `spectral_preserving_melin_superposition` | 中高 | 谱点保持的 Mellin 叠加，强插值断言 |
| 63 | `melin_zero_localization` | 中高 | Mellin 零点局部化，强插值断言 |
...
mollified_melin_separation (定理，已从公理降级)
  ...
  │   └── melin_pair_sum_nondegenerate (公理 61) ← 泛函分析，与ζ无关
  └── spectral_preserving_perturbation_build (定理，已从公理降级)
      ├── spectral_preserving_melin_superposition (公理 62) ← 插值理论
      └── melin_zero_localization (公理 63) ← 插值理论
```
       ...
       └─ off_critical_line_contradiction (定理)
            └─ pair_contribution_sigma_dependent (定理)
                 ...
                 └─ mollified_spectral_preserving_perturbation (定理)
                      ├─ zero_side_melin_localization (公理 60)
                      └─ mollified_melin_separation (定理)
                           ├─ pair_nonzero_kernel_exists (定理)
                           │   └─ melin_pair_sum_nondegenerate (公理 61)
                           └─ spectral_preserving_perturbation_build (定理)
                                ├─ spectral_preserving_melin_superposition (公理 62)
                                └─ melin_zero_localization (公理 63)

spectral_zero_equality (定理)
  └─ mollified_trace_equality (定理)
       ├─ arthur_trace_formula (定理)
       ├─ mollified_continuous_spectrum_vanishes (公理 57)
...
编译器现在阻止：
- 把三维特征函数传给二维 Laplacian
- 把二维函数传给 Shimura 提升的输出位置
- 把 `ℝ` 当作流形点传入热核
...
| 第二档：已知大定理 | ~25 | 40% | ZFC 可证，需引用专业文献 |
| 第三档：高风险/分析缺口 | 7 | 12% | 真正的未证明断言 |

### 距离 ZFC 下证明 RH 的三个层次

**层次 1（已完成）**：修正错误公理
- `mollified_continuous_spectrum_vanishes`：澄清为磨光函数下成立，非过度强化
- `mollified_spectral_delta`：保留为合理插值断言
- `delta_trace_zero_correspondence`：标记为高风险，需单点 Mellin 局部化

**层次 2（进行中）**：证明 5 条分析公理
- `zero_side_melin_localization`、`melin_pair_sum_nondegenerate`、`spectral_preserving_melin_superposition`、`melin_zero_localization`、`delta_trace_zero_correspondence`
...
**形式化链条闭合（0 sorry，RH 主定理已证），63 条公理中约 88% 是 ZFC 标准结果或已知大定理，核心分析缺口集中在 5 条 Mellin 插值公理上。若以"ZFC 下可证"为标准，完成度约 60-70%。**
...
### 优先级 1：MEF 分析核心
- [ ] 拆 `delta_trace_zero_correspondence`（引入单点 Mellin 局部化公理）
- [ ] 统一处理 Mellin 变换插值理论基础设施
- [ ] 验证 `spectral_preserving_melin_superposition` 的数学合理性
...
*文档生成时间: 2026-09-10*  
*对应文件: stage_4.lean (1924 行, 63 公理, 58 定理, 35 opaque, 0 sorry)*
