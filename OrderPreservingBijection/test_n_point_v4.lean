...
theorem mellin_n_point_interpolation (n : ℕ) (s : Fin n → ℂ)
  ...
  have hV_det_ne_zero : V.det ≠ 0 := by
    ...
    intro h4
    have h5 : v j = v i := sub_eq_zero.mp h4
    exact h_vj_ne_vi h5
  have hD_det_ne_zero : D.det ≠ 0 := by
    ...
    have h1 : v j - 1 ≠ 0 := by
      intro h
      exact hv_ne_one j (sub_eq_zero.mp h)
    have h3 : s j ≠ 0 := h_ne_zero j
    exact div_ne_zero h1 h3
  have hA_det_ne_zero : A.det ≠ 0 := by
    rw [hA_eq_VD, Matrix.det_mul]
    have h_mul : V.det * D.det ≠ 0 := mul_ne_zero hV_det_ne_zero hD_det_ne_zero
    exact h_mul
  have hBA : A⁻¹ * A = 1 := Matrix.nonsing_inv_mul A hA_det_ne_zero
  let c : Fin n → ℂ := (A⁻¹).mulVec w
